...
let spelasIntervals;
const krasas = [
    "lime", "red", "blue", "yellow", "purple", "orange", "pink",
    "cyan", "magenta", "white", "gray", "brown", "gold", "aqua", "green"
];
let cuskaKrasa = localStorage.getItem("cuskaKrasa") || "lime";
...
function irAtlautaKrasa(krasa) {
    return krasas.includes(krasa);
}
function mainitCuskasKrasu() {
    let krasa = document.getElementById("krasaIevade").value.toLowerCase();
    if (isValidColor(krasa) && irAtlautaKrasa(krasa)) {
        cuskaKrasa = krasa;
        document.getElementById("krasa").innerHTML =
            `Čūskas krāsa nomainīta uz <span style="color:${cuskaKrasa}">${cuskaKrasa}</span>`;
        localStorage.setItem("cuskaKrasa", cuskaKrasa);
    } else {
        document.getElementById("krasa").innerHTML =
            `Nederīga krāsa: <span style="color:red">${krasa}</span><br>Atļautās: ${krasas.join(", ")}`;
    }
}
window.addEventListener("load", () => {
    const ievade = document.getElementById("krasaIevade");
    const krasaInfo = document.getElementById("krasa");

    if (ievade) {
        ievade.value = cuskaKrasa;
        krasaInfo.innerHTML =
            `Čūskas krāsa uzstādīta uz <span style="color:${cuskaKrasa}">${cuskaKrasa}</span>`;
        ievade.addEventListener("input", mainitCuskasKrasu);
    }
});
